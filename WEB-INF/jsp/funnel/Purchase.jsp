...
<neon:funnelPage title="Billing Information" rightSidebar="PurchaseSidebar.jsp">
...
<div id="title">
<img src="images/common/billing-information2.gif" alt="Billing Information! Sign Up Today! It's Easy, Safe and Secure" /></div>
...
<c:url var="purchaseUrlIntl" value="purchaseSignup.do?i18n=1"/>
</c:if>
<c:if test="${command.search}">
<c:url var="purchaseUrlIntl" value="purchaseSearch.do?i18n=1"/>
</c:if>
<li>
...
<c:if test="${command.signup}">
	<li><label>Payment Plan: </label> $19.95 per year </li>
	<input type="hidden" name="rateId" value="85"/>
			
	<li class="checkbox"><label></label><form:checkbox path="recurringBilling" cssClass="formfield" /> 
    <strong>Yes!</strong> I would like the convenience of recurring billing.</li>
<li class="instructions"><label></label>You may cancel at any time.</li>
...
<c:if test="${command.search}"><c:set var="agreementVersion" value="funnel"/></c:if>
<c:if test="${command.signup}"><c:set var="agreementVersion" value="members"/></c:if>

                    <li class="checkbox"><label></label><form:checkbox path="acceptAgreement" cssClass="formfield" /> 
					I have read &amp; agree to the <a href="${showAgreementUrl}?version=${agreementVersion}" target="_blank">User Agreement</a></li>
                </ul>
                
                <ul id="submit">
                    <li><label></label><input type="image" src="images/common/bt-continue-to-search.png" alt="Continue To Your Search" /></li>
					<li><div align="left" class="small"><div align="center">
