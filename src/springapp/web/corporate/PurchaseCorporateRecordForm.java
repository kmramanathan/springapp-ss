...
public class PurchaseCorporateRecordForm extends AbstractFunnelController  {
	
	// protected Logger logger = Logger.getLogger(getClass());
		
	@RequestMapping(value="/corporate/purchaseCorporateRecordSearch.do", method = RequestMethod.GET)
	public String setupFormSearch(
		...
		logger.info("Purchase Search Form Loading => Corporate search record form: "+searchcrfc);
		...
		PurchaseCorporateRecordFormCommand cmd = (PurchaseCorporateRecordFormCommand) session.getAttribute("purchaseCorporateRecordFormCommand");
		if (cmd == null) {
			cmd = new PurchaseCorporateRecordFormCommand();
		} else {
			// clear CC info
			cmd.setCcNumber("");
			cmd.setCcAuthCode("");
	...
	private void setDefaults(PurchaseCorporateRecordFormCommand cmd) {		
		if (!cmd.getI18n()) {
	...
	private void setSample(PurchaseCorporateRecordFormCommand cmd) {
		...
		cmd.setCountryCode("US");
		cmd.setEmail("d.shahul@yahoo.co.in");
	...
	@RequestMapping(value="/corporate/purchaseCorporateRecordSearch.do", method = RequestMethod.POST)
	public String processSubmitSearch(
			HttpSession session,
			ModelMap map,
			@ModelAttribute("command") PurchaseCorporateRecordFormCommand command,
			Errors errors,
			SessionStatus status) {
		SearchCorporateRecordFormCommand searchcrfc = (SearchCorporateRecordFormCommand) session.getAttribute("searchCorporateRecordFormCommand");
		logger.info("Purchase Search Form Loading =>Corporate search record form - POST: " + searchcrfc);
		...
		if (errors.hasErrors()) {
			...
			session.setAttribute("purchaseCorporateRecordFormCommand", command);
			status.setComplete();
			return "redirect:confirmCorporateSearch.do";
		}
	}
	
	public static class PurchaseCorporateRecordFormCommand {
		private String name;
	...
	public class PurchaseFormValidator extends NeonValidator implements Validator {
		public boolean supports(Class clazz) {
			return clazz.equals(PurchaseCorporateRecordFormCommand.class);
		}
		
		public void validate(Object target, Errors errors) {
			PurchaseCorporateRecordFormCommand cmd = (PurchaseCorporateRecordFormCommand) target;
