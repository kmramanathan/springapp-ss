...
public class testParser
{
	...
	public static void main(String args[])
	{
		...
		try
		{
			...
			String key = "one";
			String resType = "FP";
			System.out.println("Key for Request XML :: "+key);
			String strXML = getXmlRequest(resType,key);
			...
			System.out.println("Result XML :"+resXML);
			//wrinting into file
		//	File BGresult=new File("c:/fpdetail.txt");
		...
		//	p.print(resXML);
			...
			System.out.println("Received At :"+receivedAt);
			...
			{		
				...
				System.out.println(BGResultDetail(nList));				
			}
			else
			{
				...
				if(total > 0){	fpResultDetail(nList, total); }
	...
	public static String getXmlRequest(String resType, String key)
	{
		...
		if(resType.equals("FP"))
		{
			...
			strXML = strXML+ "<firstName>john</firstName>" ;
			strXML = strXML+ "<middleName></middleName>";
			strXML = strXML+ "<lastName>david</lastName>" ;
			strXML = strXML+ "<city></city>" ;
			strXML = strXML+ "<state>DC</state>";
			strXML = strXML+ "<DOB></DOB>" ;
			strXML = strXML+ "</searchParams>" ;
		...
		if(resType.equals("BG"))
		{
			...
			try
			{
				...
				strXML= "<FDSRequest>";
				strXML = strXML+ "<username>searchsystems</username>";
				strXML = strXML+ "<password>xml154g</password>";
				strXML = strXML+ "<sType>PRO</sType>";
				strXML = strXML+ "<detail>1</detail>";
				strXML = strXML+ "<testmode>false</testmode>";
				strXML = strXML+ "<searchParams>" ;
				strXML = strXML+ "<lastName>dickson</lastName>" ;
			...
			{
				System.out.println("Exception occured :"+e.getMessage()+" cause :"+e.getCause()+" class :"+e.getClass());
			}
		}
	...
	//getting the teaser Result
	public static Hashtable fpResultDetail(NodeList nList, int total)
	{
		...
		for(int i=0;i<total;i++)
		{
			...
			fpDetail1 fp = new fpDetail1(readXMLNode(firstNode,"ValidityDate"),
			...
			//getting and setting Age for teaser
		...
		/*	Enumeration keys = ageHash.keys();
			String key ="";
			 while(keys.hasMoreElements())
				{
					key=(String)keys.nextElement();
					String age=(String)ageHash.get(key);
					System.out.println("Aged IDs are :"+key);
					System.out.println("Age is :"+age);
				 }
			fpResult.put(readXMLNode(firstNode,"personID"), fp);
			
			//Doing DOB validation here
		...
		/*	System.out.println("DOB is "+DOB);
			if(!DOB.equals(""))
			{
				...
				if(DOBToken.length > 1)
				{
					String month = DOBToken[0];
					String date = DOBToken[1];
					String year = DOBToken[2];
					System.out.println("Month :"+month+" Date :"+date+" Year :"+year);
					if(month.equals("00") || date.equals("00")){
	...
	public static class fpDetail1 {
		...
		public fpDetail1(String validityDate, String lastName, String firstName,
