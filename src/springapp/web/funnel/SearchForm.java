...
public class SearchForm extends AbstractFunnelController {
	...
	@RequestMapping(value = "/funnel/searchBasic.do", method = RequestMethod.GET)
	public String setupForm(
		...
		return setupFormBGC(session, map, test, sample, nationwide, vwSearch, CAMPAIGN_OLD_PRICES);
	}
	
	@RequestMapping(value = "/funnel/searchFull.do", method = RequestMethod.GET)
	public String setupFormFull(
		...
		return setupFormBGC(session, map, test, sample, nationwide, vwSearchBGCFull, CAMPAIGN_OLD_PRICES);
	}
	 ...
	 * set rate id (item) 
	 */
	@RequestMapping(value = "/funnel/search.do", method = RequestMethod.GET)
	public String setupFormCampaign(
			HttpSession session,
			ModelMap map,
			@RequestParam(value="test",required=false) Boolean test,
			@RequestParam(value="sample",required=false) Boolean sample,
		...
		return setupFormBGC(session, map, test, sample, nationwide, vwSearchBGCFull, campaignId);		
	}
	...
	@RequestMapping(value = "/funnel/searchBasic.do", method = RequestMethod.POST)
	public String processSubmit(
		...
		return this.processSubmitBGC(session, map, sfc, errors, status, vwSearch);
	}
	
	@RequestMapping(value = "/funnel/search.do", method = RequestMethod.POST)
	public String processSubmitFull(
			HttpSession session,
			ModelMap map,
			@ModelAttribute("command") SearchFormCommand sfc,
			Errors errors,
			SessionStatus status) {
		return this.processSubmitBGC(session, map, sfc, errors, status, vwSearchBGCFull);
	}
	
	@RequestMapping(value = "/funnel/searchCampaign.do", method = RequestMethod.POST)	
	public String processSubmitCampaign(
			HttpSession session,
			ModelMap map,
			@ModelAttribute("command") SearchFormCommand sfc,
			Errors errors,
		...
		if (errors.hasErrors()) {
			...
			case CAMPAIGN_NEW_PRICES:
				if (sfc.getNationwideSearch()) {
					...
					sfc.setPrice(new BigDecimal("7.95"));
				}
				break;
			}
	...
	@RequestMapping("/funnel/searchAgain.do")
	public String reset(HttpServletRequest request, HttpSession session) {		
		...
		String redir = searchRedir + "?nationwide=" + nationwide + "&campaign=" + campaign;
		return redir;
	}
